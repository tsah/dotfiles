...
2. Run `bun run check` in `wayfinder/`, and bundle-check `pi/extensions/tmux-worker-lifecycle.ts` with the Pi package import marked external.
3. Run `bun test` and `scripts/qa` in `wayfinder/`. The latter must use only its private nested tmux sockets and an owned `/tmp/qa-alt-k-*` root, and any lineage checks must keep their SQLite store under that same disposable root. Verify `ALT_K_TUI_QA_ROOT` rejects paths outside that pattern and refuses to replace an existing root without its `.alt-k-qa-owned` marker.
4. Run `nvim --headless '+lua require("pi_tmux").setup()' +qa`. Then grep `wayfinder/src/workflow.ts`, `wayfinder/src/agent-api.ts`, `pi/extensions/tmux-worker-lifecycle.ts`, and `nvim/lua/pi_tmux.lua` and verify they contain no `send-keys`, `load-buffer`, `paste-buffer`, or `capture-pane`; the isolated TUI QA driver may still use those commands to drive and inspect its disposable terminal.
...
9. Run Pi with `--wait`; verify output arrives only after the report's `settledGeneration` advances on `agent_settled`, the structured result matches that generation, and the successful window remains available. Set `DOTFILES_WORKER_WAIT_TIMEOUT=1` for a long prompt; verify timeout is nonzero and the worker remains attachable. Run Claude with `--wait` and verify it settles from hook reports without claiming `send` or `result` support. Do not expect OpenCode wait to work without a verified report producer.
...
1. Run `python3 -m unittest discover -s wayfinder/tests`; all `/proc`, tmux, and signal operations must remain fake and must never target real processes.
2. Verify sustained low `MemAvailable` plus memory PSI is required, focused panes and ambiguous identities are rejected, settled agents are preferred, largest RSS breaks equal-state ties, and only one action is allowed per boot.
3. Seed disposable incident files and verify Wayfinder renders a red `×` with `terminated` or `lost on reboot`, keeps the incident subtree prominent and expanded, and retains the underlying worktree target.
4. In disposable state, change the fake boot ID after an unclean snapshot and verify only prior working/blocked/unknown agents produce idempotent loss incidents; a clean shutdown must produce none.
5. Run `systemd-analyze --user verify systemd/user/wayfinder-resource-guard.service`. Never lower thresholds or run a pressure scenario against the real machine as QA.

## Headless Waystation agent API

1. In the disposable tmux server, create tagged Pi, Claude, and OpenCode panes. Run `waystation agent list` and verify each receives a unique `ws-…` ID in pane option `@waystation_agent_id`; repeat listing and rename the session to verify IDs stay stable. Verify `--cwd` excludes agents from another canonical worktree.
2. Run `status` and `capabilities` for every ID. Pi with a live lifecycle socket must advertise report wait, Unix-socket send, and report result. Claude/Codex must advertise report wait while explicitly rejecting `send` and `result`; OpenCode must advertise monitor-only capabilities. No command may fall back to tmux input or pane capture.
3. Start Pi with the global lifecycle extension and inspect its advertised socket. Verify it has a randomized agent-bound name, is owned by the current UID, mode `0600`, removed on shutdown, and receives newline-delimited JSON carrying the expected agent ID. Send while idle, busy with the default follow-up, explicit `--delivery follow-up`, and explicit `--delivery steer`; verify each message appears as a native Pi user message. Attempt a second send before settlement and verify it is rejected rather than sharing another request's result.
4. Record `settledGeneration`, run `waystation agent send ID --wait`, and verify wait returns only on a newer settled generation. Send at least two more turns, retrieve latest and explicit historical generations with `result`, and verify replies/statuses are not confused across turns. Verify a failed/aborted Pi generation produces a nonzero structured result.
5. Exercise report-only wait with synthetic Claude/Codex reports: an idle/working/blocked report must not falsely settle, a newer done report must settle, and `--after N` must ignore an older completion. Verify timeout is nonzero and nondestructive.
6. Exercise compatibility boundaries: `dotfiles-workflow agents`, `dotfiles-workflow send --pane`, worker/agent launchers, and bare `waystation` still work. Confirm `dotfiles-workflow send --no-submit` fails explicitly rather than editing a terminal buffer.
...
8. Press Alt-R on the selected disposable tmux session row, paste a new name, and confirm. Verify the row updates, remains selected, included agents remain visible, path/common-dir tags are unchanged, and subsequent session ensure returns the renamed session without creating a duplicate. Repeat on a nested lineage child session row and verify the same guarantees hold. Cancel a second rename and verify no change. Select a nested workspace row, press Alt-L, and verify it remains selected as a new root with its descendants still attached; confirm SQLite and its manifest store a null parent and its tmux parent option is absent. Press Alt-A, type to filter the valid parent list, and reattach it; verify the source remains selected beneath the chosen parent, its descendants stay attached, and SQLite, the manifest, and tmux mirror agree. Confirm the source, its descendants, and any cross-repository workspaces are not offered as parents, Escape cancels without mutation, and lineage actions are unavailable on detail rows.
...
2. Exercise `AgentSendReference`, `AgentSendContents`, and `AgentAppendContext` over no range and a visual range. Verify default payload includes path and range. Verify append submits the context as a native follow-up.
...
5. Confirm delivery calls `waystation agent send` with the selected stable ID, uses Pi's native Unix-socket transport, reports unsupported harness/socket failures, and never invokes tmux input or pane capture. Confirm `AgentAppendContext` is a submitted native follow-up rather than an unsubmitted terminal-editor paste.
