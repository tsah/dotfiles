...
4. Seed a completion report and verify it renders green `✓ ready`. Focus that exact child, reopen Alt-K, and verify it renders blue `○ idle`; emit a report with a newer `updatedAt` and verify it returns to `ready`. Verify red `! waiting`, orange `● working`, green `✓ ready`, blue `○ idle`, purple `? unknown`, aggregate precedence, and legacy `running`/`attention` report compatibility.
