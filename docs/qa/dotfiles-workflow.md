...
3. Verify sessions are sorted upward from the bottom prompt, trees with one to three direct expanded items begin expanded, and larger trees begin collapsed. Verify lineage renders as a real recursive hierarchy rather than marker-only hints: each session row stays selectable, its own details stay immediately beneath it, and nested child sessions stay contiguous inside the same subtree. Exercise Left/Right fully: Right expands the selected session; Left on a detail jumps to its owning session; Left on an expanded session collapses it; Left on a collapsed nested session jumps to its lineage parent. Select a plain window, Pi, Claude, Codex, and OpenCode child independently and verify Enter focuses the exact target. Search for text present only in a collapsed child and verify its full ancestor chain remains visible. Search by workspace ID, parent workspace ID, child-count lineage text, and lineage branch/session names and verify the same parent rows remain discoverable without ordering regressions. In the disposable fixture, record durable activity for one worktree and modify a file in another, restart without their tmux sessions, and verify both sort ahead of an old clean worktree with `[source age]` recovery labels.
...
10. Capture a deterministic visual hierarchy from the safe fixture without touching real tmux state:
   ...
   ```
   Then inspect the live render with `tmux -S /tmp/qa-alt-k-lineage/outer.sock attach -t harness` or read the captured pane text at `/tmp/qa-alt-k-lineage/lineage-render.txt`. Confirm the `idp-java` subtree visibly contains `fix-coverage` and `fix-java` with tree connectors and nested detail rows.
